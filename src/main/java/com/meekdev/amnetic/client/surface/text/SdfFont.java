...
public final class SdfFont {
    ...
    public record Glyph(int ax, int ay, int aw, int ah, float xoff, float yoff, float advance) {}

    private final ByteBuffer fontData; // kept alive, stb reads from it on every bake
    ...
    private SdfFont(ByteBuffer fontData, STBTTFontinfo info, float scale, float ascent, int texture) {
    ...
    public float kern(int cp1, int cp2) {
        return stbtt_GetCodepointKernAdvance(info, cp1, cp2) * scale;
    }
    ...
    private Glyph bake(int cp) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            ByteBuffer sdf = stbtt_GetCodepointSDF(info, scale, cp, PADDING, ONEDGE, PIXEL_DIST_SCALE, w, h, xo, yo);
            if (sdf == null) { // no shape (space etc), advance only
                Glyph g = new Glyph(0, 0, 0, 0, 0, 0, advance);
                glyphs.put(cp, g);
                return g;
            }
    ...
    // null (logged) if the font can't be read, render thread
    static SdfFont load(Identifier fontId) {
        ...
        try (InputStream in = Minecraft.getInstance().getResourceManager().getResource(fontId)
            ...
            fontData = null; // ownership moves to the font, freed never (lives for the session)
            return new SdfFont(kept, info, scale, ascent, tex);
        } catch (Exception e) {
